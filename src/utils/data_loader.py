...
class TrafficLoader:
    ...
    def load_data(self, path=None):
        ...
        By default the loader will look for `data/traffic.csv` in the project
        root (two levels above this file). You can also override the path by
        passing `path` or setting the `FLARE_DATA_PATH` environment variable.
        """
        # Determine default path relative to repository root
        if path is None:
            project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
            default_path = os.path.join(project_root, 'data', 'traffic.csv')
            path = os.environ.get('FLARE_DATA_PATH', default_path)

        if not os.path.exists(path):
            raise FileNotFoundError(f"Data file not found at {path}. Please run download_data.py or set FLARE_DATA_PATH.")
