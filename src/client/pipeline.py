...
class FlarePipeline:
    def __init__(self, model_path=None, scaler_path=None, input_dim=13):
        # Compute repository root (two levels up from this file)
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))

        default_model = os.path.join(project_root, 'src', 'client', 'autoencoder.pth')
        default_scaler = os.path.join(project_root, 'src', 'client', 'scaler.pkl')

        # Allow environment overrides, function arg overrides, then defaults
        self.model_path = os.environ.get('FLARE_MODEL_PATH', model_path or default_model)
        self.scaler_path = os.environ.get('FLARE_SCALER_PATH', scaler_path or default_scaler)
    ...
    def load_artifacts(self):
        ...
        if os.path.exists(self.model_path):
            ...
            raise FileNotFoundError(f"Model not found at {self.model_path}. Run training (src/client/train.py) to create this artifact.")
            
        # Load Scaler
        try:
            self.preprocessor.load(self.scaler_path)
        except FileNotFoundError:
             raise FileNotFoundError(f"Scaler not found at {self.scaler_path}. Run training (src/client/train.py) to create this artifact.")
