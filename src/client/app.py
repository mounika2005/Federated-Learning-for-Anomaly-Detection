...
# When running this file directly (python src/client/app.py) the package
# imports like `from src.utils...` can fail because the project root is not
# on `sys.path`. Add the project root (two levels up from this file) so
# absolute imports using the `src` package work both when run as a module
...
def start_client():
    # Preferred: convert NumPyClient to the newer Client interface and start it.
    # This follows Flower's recommended migration path: NumPyClient().to_client()
    client_np = FlareClient()
    try:
        client_new = client_np.to_client()
        fl.client.start_client(server_address="127.0.0.1:8080", client=client_new)
    except Exception:
        # Older Flower versions may not have `to_client()` or `start_client()`;
        # fall back to the legacy helper to preserve compatibility.
        fl.client.start_numpy_client(server_address="127.0.0.1:8080", client=client_np)
